    ...
    async execute(regexMatches, guildID, configValue, discordBot){
        try {
            ...
            if(discordBot.guilds.cache.get(guildID).channels.cache.get(upcomingPvEBossAlertChannelID) !== undefined){
                if(discordBot.guilds.cache.get(guildID).channels.cache.get(upcomingPvEBossAlertChannelID).permissionsFor(discordBot.user.id).has('ViewChannel') === true){
                    if(discordBot.guilds.cache.get(guildID).channels.cache.get(upcomingPvEBossAlertChannelID).permissionsFor(discordBot.user.id).has('SendMessages') === true){
