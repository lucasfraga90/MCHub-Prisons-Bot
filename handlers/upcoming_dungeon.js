    ...
    async execute(regexMatches, guildID, clientID, configValue, discordBot){
        try {
            ...
            if(discordBot.guilds.cache.get(guildID).channels.cache.get(upcomingDungeonAlertChannelID) !== undefined){
                if(discordBot.guilds.cache.get(guildID).channels.cache.get(upcomingDungeonAlertChannelID).permissionsFor(clientID).has('ViewChannel') === true){
                    if(discordBot.guilds.cache.get(guildID).channels.cache.get(upcomingDungeonAlertChannelID).permissionsFor(clientID).has('SendMessages') === true){
